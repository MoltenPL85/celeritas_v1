...
func (m *Mail) SendUsingAPI(msg Message, transport string) error {
	...
	driver, err := m.newClient(transport, cfg)
	if err != nil {
		return err
	}
...
func (m *Mail) newClient(driver string, cfg apimail.Config) (apimail.Mailer, error) {
	switch driver {
	case "mailgun":
		return drivers.NewMailgun(cfg)
	case "sparkpost":
		return drivers.NewSparkPost(cfg)
	case "sendgrid":
		return drivers.NewSendGrid(cfg)
	}
